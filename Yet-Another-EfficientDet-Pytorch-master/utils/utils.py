...
def display_gt(annot, imgs, imshow=True, imwrite=False):
    for i in range(len(imgs)):
        if len(annot[i]) == 0:
            continue
        ...
        for j in range(annot[i].shape[0]):
            (x1, y1, x2, y2, _) = annot[i][j]
            obj = 'wheat'
            # obj = obj_list[preds[i]['class_ids'][j]]
            score = 1
        ...
        if imwrite:
            os.makedirs('test/annot', exist_ok=True)
            cv2.imwrite(f'test/annot/{i}.jpg', imgs[i])
