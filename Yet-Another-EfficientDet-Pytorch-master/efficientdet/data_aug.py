...
class ConvertFromInts(object):
    def __call__(self, sample):
        ...
        return {'img': img.astype(np.float32), 'annot': annot.astype(np.float32)}

class ConvertFromFloats(object):
    def __call__(self, sample):
        img, annot = sample['img'], sample['annot']
        return {'img': img.astype(np.int32), 'annot': annot.astype(np.int32)}
...
class SubtractMeans(object):
    ...
    def __call__(self, sample):
        ...
        img -= self.mean
        return {'img': img.astype(np.float32), 'annot': annot}
...
class HorizontalFlip(object):
    def __call__(self, sample):
        ...
        img = cv2.flip(img, 1)
        height, width, channels = img.shape
        annot[:, 0] = width - annot[:, 0]
        annot[:, 2] = width - annot[:, 2]
        return {'img': img, 'annot': annot}
...
class VehicleFlip(object):
    def __call__(self, sample):
        ...
        img = cv2.flip(img, 0)
        height, width, channels = img.shape
        annot[:, 1] = height - annot[:, 1]
        annot[:, 3] = height - annot[:, 3]
        return {'img': img, 'annot': annot}
